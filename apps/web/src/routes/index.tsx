    ...
    <>
      ...
      <Section title="System">
        <dl className="grid grid-cols-2 gap-x-8 gap-y-1 border-t border-[#1f1f1f] pt-2 sm:grid-cols-3 lg:grid-cols-6">
          ...
          <Metric
            label="Disk"
            value={stats && stats.disk_total > 0 ? `${bytes(stats.disk_used)} / ${bytes(stats.disk_total)}` : '-'}
          />
      ...
      <Section title="Deployments" description="most recent across all projects">
        {info.isLoading ? (
          ...
          <Empty>
            No deployments yet. <Link to="/projects" className="underline">Create a project</Link> to get started.
          </Empty>
        ) : (
          <Table head={['', 'Project', 'Commit', 'Trigger', 'When']}>
            {info.data?.recent_deployments.map(({ deployment, project_name }) => (
              <Row key={deployment.id}>
                ...
                <Cell>
                  <Link to="/projects/$projectId" params={{ projectId: deployment.project_id }} className="hover:underline">
