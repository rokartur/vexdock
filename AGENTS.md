...
`docker` or `system` in `components/shell.tsx` if it belongs in the section bar
— the breadcrumb derives itself from the URL. A section of a page is a child route, never a `?tab=`:
...
its `actions`; both are rendered up into the shell's bars, so a page never draws
chrome of its own. `Tabs` moves through the URL, `Segmented` moves a value; never
