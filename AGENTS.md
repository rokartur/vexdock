...
`docker` or `system` in `components/shell.tsx` if it belongs in the nav bar.
`home` is spelled out across the bar, the other two are its menus, and the
...
`filters`, actions in its `actions`; the actions ride up into the shell's
workspace bar and the rest becomes the page's own first row, so a page never
