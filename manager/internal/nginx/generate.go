...
type Upstream struct {
	...
	CertDir string
	// Redirects are tried in order against the full request URL.
	Redirects []Redirect
	// BasicAuth turns on auth_basic against AuthFileName(Hostname).
	BasicAuth bool
}

// Redirect answers a request whose URL matches Regex with a redirect to
// Replacement, unless that is the URL already asked for. Both are validated by
// security.ValidateRedirect before they get here.
...
// AuthFileName is the htpasswd file beside a hostname's vhost.
func AuthFileName(hostname string) string { return hostname + ".htpasswd" }

// generatedRoot is where nginx sees the generated directory.
const generatedRoot = "/etc/nginx/generated"
...
// accessRules is the redirects and basic auth of a proxied location. Both
// stay out of the ACME location so issuance keeps working behind them.
// A redirect whose result equals the request URL is skipped, so a www preset
// does not loop on the www host.
func accessRules(u Upstream) string {
	var b strings.Builder
	if u.BasicAuth {
		b.WriteString("        auth_basic \"Restricted\";\n")
		fmt.Fprintf(&b, "        auth_basic_user_file %s/%s;\n", generatedRoot, AuthFileName(u.Hostname))
	}
	if len(u.Redirects) > 0 {
		b.WriteString("        set $redirect_url $scheme://$host$request_uri;\n")
	}
	for _, r := range u.Redirects {
		...
		b.WriteString("        set $redirect_to \"\";\n")
		fmt.Fprintf(&b, "        if ($redirect_url ~ \"%s\") { set $redirect_to \"%s\"; }\n", r.Regex, r.Replacement)
		b.WriteString("        if ($redirect_to = $redirect_url) { set $redirect_to \"\"; }\n")
		fmt.Fprintf(&b, "        if ($redirect_to) { return %d $redirect_to; }\n", status)
	}
	if b.Len() > 0 {
		b.WriteString("\n")
	}
...
func proxyLocation(u Upstream) string {
	var b strings.Builder
...
// HashPassword returns an nginx {SSHA} hash: base64 of SHA-1(password+salt)
// followed by the salt. nginx checks it on every request, so a slow hash would
// cost CPU on each one.
func HashPassword(password string) string {
	salt := make([]byte, 8)
	_, _ = rand.Read(salt)
	sum := sha1.Sum(append([]byte(password), salt...))
	return "{SSHA}" + base64.StdEncoding.EncodeToString(append(sum[:], salt...))
}
