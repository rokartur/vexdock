...
// Visitor identifies a device for one site for one day, and nothing beyond it.
//
// The coarse client bucket rather than the raw user agent, and an IPv6 /64
// rather than the exact address, because everything finer splits one person
// into several: a private window, a browser that updated its version mid-day,
// or an operating system rotating its temporary IPv6 address all look like a
// new user agent or a new address while being the same device.
// ponytail: two devices behind one NAT with the same browser and OS count as
// one. Fixing that needs something stored on the device, which is the cookie
// this feature exists to avoid.
func Visitor(now time.Time, hostname, ip string, client Client) string {
	sum := sha256.New()
	sum.Write(saltFor(now))
	sum.Write([]byte(hostname + "|" + NormalizeIP(ip) + "|" + client.Device + "|" + client.Browser + "|" + client.OS))
	return hex.EncodeToString(sum.Sum(nil))[:16]
}

// NormalizeIP collapses the addresses one device answers from into a single
// key: an IPv4-mapped address becomes its IPv4, and an IPv6 address keeps only
// the /64 prefix, since the half below it is regenerated by the device itself.
