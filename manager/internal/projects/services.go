...
func (s *Service) CreateService(ctx context.Context, p *database.Project, in ServiceInput) (*database.Service, error) {
	name := strings.TrimSpace(in.Name)
	if err := security.ValidateServiceName(name); err != nil {
		return nil, err
	}
	if existing, err := s.db.ServiceByName(ctx, p.ID, name); err == nil && existing != nil {
		return nil, fmt.Errorf("this project already has a service named %q", name)
	}
	...
	case database.ServiceImage:
		...
		rendered, err := engines.Render(engines.Spec{
			...
			EnvFile: s.ServiceEnvFilePath(p, name),
		})
		if err != nil {
			return nil, err
		}
	...
	if len(seed) > 0 {
		...
		if err := s.SetServiceEnvironment(ctx, svc.ID, vars); err != nil {
	...
	if _, err := s.WriteOverlay(ctx, p); err != nil {
		_ = s.db.DeleteService(ctx, svc.ID)
		return nil, err
	}
	return svc, nil
...
func (s *Service) DeleteService(ctx context.Context, svc *database.Service, p *database.Project) error {
	if !svc.Managed() {
	...
	if err := os.Remove(s.ServiceEnvFilePath(p, svc.ComposeServiceName)); err != nil && !os.IsNotExist(err) {
		return err
	}
	_, err := s.WriteOverlay(ctx, p)
	return err
}
